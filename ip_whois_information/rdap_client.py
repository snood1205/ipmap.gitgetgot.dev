...
class RDapClient:
    @staticmethod
    def fetch_rdap(ip_address):
        validate_ipv4_address(ip_address)
        url = f'https://rdap.org/ip/{ip_address}'
        json_response = get(url).json()
        ip_info = RDapClient.__create_ip_info_from_json_response(json_response)
        ip_info.save()
        return ip_info
    ...
    def __create_ip_info_from_json_response(json_response):
        ...
        return IPInfo(
            ...
            registration_date=next(
                (event["eventDate"][:10] for event in json_response.get('events') if
                 event["eventAction"] == "registration"), None),
            updated_date=next(
                (event["eventDate"][:10] for event in json_response.get('events') if
                 event["eventAction"] == "last changed"), None),
            country=json_response.get('country'),
            ref_url=next((link["href"] for link in json_response.get("links") if link["rel"] == "self"), ""),
